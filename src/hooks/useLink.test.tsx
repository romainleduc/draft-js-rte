...
const Wrapper = ({ editorState: editorStateProps, children }: { editorState: EditorState, children: React.ReactNode }): JSX.Element => {
...
beforeEach(() => {
  ...
  const blocksFromHTML = convertFromHTML('Editor state with link to <a href="https://draftjs.org/">Draft.js</a>');
...
const renderLinkHook = (editorState: EditorState): RenderHookResult<{}, UseLinkResult> => {
  return renderHook(() => useLink(), { wrapper: ({ children }) => (
...
describe('states', () => {
  test('should be able to retrieve the selected link when the selection is reduced', async () => {
    const { result } = renderLinkHook(EditorState.set(editorStateWithContent, { selection: editorStateWithContent.getSelection().merge({ anchorOffset: 28, focusOffset: 28 })}));
...
describe('toggle', () => {
  test('should toggle the selection range link', async () => {
    const blockKey = editorStateWithContent.getCurrentContent().getFirstBlock().getKey();
    const { result } = renderLinkHook(EditorState.set(editorStateWithContent, { selection: new SelectionState({
      anchorOffset: 0,
      anchorKey: blockKey,
      focusOffset: 6,
      focusKey: blockKey,
